...
" Original: /(\%(\_[^()]\+\|(\_[^()]*)\)*)\_s*=>/ -- the (a+)* shape lets
" the outer * re-partition one run of non-paren chars across the \+
" branch in exponentially many ways, so a paren group with nested char
" classes before '=>' backtracks catastrophically. Hoisting the star
" outside the alternation matches the identical language in linear time
" (verified: identical synstack spans across typed/untyped/nested-paren-
" default arrow functions and negative cases -- ternaries, object
" literals -- against the original pattern).
...
" Same ReDoS shape and same fix as typescriptArrowFuncDef above.
