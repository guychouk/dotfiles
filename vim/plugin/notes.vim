" A simple note taker, works with a local notes.md file in the pwd

function! s:new_note(first, last) abort
  let l:src = expand('%:p')
  if empty(l:src)
...
augroup notes
  autocmd!
  autocmd BufRead notes.md nnoremap <buffer> gf :call <sid>open_note()<CR>
augroup END

command! -range NewNote call <sid>new_note(<line1>, <line2>)
